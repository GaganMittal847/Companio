...
export const createChatList = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (existingChatList) {
            ...
            apiResponseDto.responseCode = HttpStatus.CONFLICT; // Use 409 Conflict
            return res.status(HttpStatus.CONFLICT).json(apiResponseDto);
        }
...
export const getChatListsByUserId = async (req: Request, res: Response): Promise<Response> => {
    const apiResponseDto = new ApiResponseDto();
    try {
